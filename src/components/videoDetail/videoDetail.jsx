...
import {FavoriteOutlined, MarkChatRead, Tag, Visibility,CheckCircle} from "@mui/icons-material"
      ...
      <Box display={'flex'} sx={{flexDirection:{xs:'column', md:'row'}}}>
        <Box width={{xs:"100%", md:'75%'}}>
          <ReactPlayer 
            ...
            <Typography variant='suptitle2'  p={2} sx={{opacity:'.7'}}>
              {renderHtml(videoDetail.snippet.description)}
            </Typography>

            <Stack direction='row' gap='20px' alignItems='center' py={2} px={2} >
            ...
            <Stack direction={'row'} py={1} px={2}>
              <Link to={`/channel/${videoDetail?.snippet?.channelId}`}>
                  <Stack direction={'row'} alignItems={'center'} gap={'5px'} marginTop={'5px'}>
                    <Avatar alt={videoDetail?.snippet?.channelTitle}
